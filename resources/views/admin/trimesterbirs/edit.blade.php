...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trimesterbir.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.trimesterbirs.update", [$trimesterbir->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('mide') ? 'has-error' : '' }}">
                ...
                <select id="mide" name="mide" class="form-control">
                    ...
                    @foreach(App\Trimesterbir::MIDE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mide', $trimesterbir->mide) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('mide'))
                    <em class="invalid-feedback">
            ...
            <div class="form-group {{ $errors->has('mesane') ? 'has-error' : '' }}">
                ...
                <select id="mesane" name="mesane" class="form-control">
                    ...
                    @foreach(App\Trimesterbir::MESANE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mesane', $trimesterbir->mesane) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('randevu_tipi') ? 'has-error' : '' }}">
                ...
                <select id="randevu_tipi" name="randevu_tipi" class="form-control">
                    ...
                    @foreach(App\Trimesterbir::RANDEVU_TIPI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('randevu_tipi', $trimesterbir->randevu_tipi) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
