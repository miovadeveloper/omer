...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.muayane.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.muayanes.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group {{ $errors->has('usg_tipi') ? 'has-error' : '' }}">
                ...
                @foreach(App\Muayane::USG_TIPI_RADIO as $key => $label)
                    <div>
                        <input id="usg_tipi_{{ $key }}" name="usg_tipi" type="radio" value="{{ $key }}" {{ old('usg_tipi', null) === (string)$key ? 'checked' : '' }}>
