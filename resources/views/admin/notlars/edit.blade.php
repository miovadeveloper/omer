...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.notlar.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.notlars.update", [$notlar->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('not') ? 'has-error' : '' }}">
                ...
                <textarea  id="not" name="not" class="form-control" value="{{ old('not', isset($notlar) ? $notlar->not : '') }}"></textarea>
                @if($errors->has('not'))
                    <em class="invalid-feedback">
                        {{ $errors->first('not') }}
                    </em>
