...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.receteitem.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.receteitems.update", [$receteitem->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('kutuadeti') ? 'has-error' : '' }}">
                ...
                <select id="kutuadeti" name="kutuadeti" class="form-control">
                    ...
                    @foreach(App\Receteitem::KUTUADETI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kutuadeti', $receteitem->kutuadeti) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('zaman') ? 'has-error' : '' }}">
                ...
                <select id="zaman" name="zaman" class="form-control">
                    ...
                    @foreach(App\Receteitem::ZAMAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('zaman', $receteitem->zaman) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('zaman'))
                    <em class="invalid-feedback">
            ...
            <div class="form-group {{ $errors->has('kullanim') ? 'has-error' : '' }}">
                ...
                <select id="kullanim" name="kullanim" class="form-control">
                    ...
                    @foreach(App\Receteitem::KULLANIM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kullanim', $receteitem->kullanim) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
