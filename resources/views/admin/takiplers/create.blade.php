...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.takipler.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.takiplers.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group d-none {{ $errors->has('takip_tipi') ? 'has-error' : '' }}">
                ...
                <select id="takip_tipi" name="takip_tipi" class="form-control" required>
                    ...
                    @foreach(App\Takipler::TAKIP_TIPI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('takip_tipi', null) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('durum') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="durum" name="durum" {{ old('durum', 0) == 1 ? 'checked' : '' }}>
                @if($errors->has('durum'))
                    <em class="invalid-feedback">
                        {{ $errors->first('durum') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('sat_emin') ? 'has-error' : '' }}">
                ...
                @foreach(App\Takipler::SAT_EMIN_RADIO as $key => $label)
                    <div>
                        <input id="sat_emin_{{ $key }}" name="sat_emin" type="radio" value="{{ $key }}" {{ old('sat_emin', null) === (string)$key ? 'checked' : '' }}>
            ...
            <div class="form-group {{ $errors->has('geb_haft_duzeltildi') ? 'has-error' : '' }}">
                ...
                @foreach(App\Takipler::GEB_HAFT_DUZELTILDI_RADIO as $key => $label)
                    <div>
                        <input id="geb_haft_duzeltildi_{{ $key }}" name="geb_haft_duzeltildi" type="radio" value="{{ $key }}" {{ old('geb_haft_duzeltildi', null) === (string)$key ? 'checked' : '' }}>
