...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trimesterikiuc.title_singular') }}
    </div>
            ...
            <div class="form-group {{ $errors->has('randevu_tipi') ? 'has-error' : '' }}">
                ...
                <select id="randevu_tipi" name="randevu_tipi" class="form-control">
                    ...
                    @foreach(App\Trimesterikiuc::RANDEVU_TIPI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('randevu_tipi', $trimesterikiuc->randevu_tipi) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
